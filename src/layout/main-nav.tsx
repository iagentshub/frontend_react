...
export function MainNav() {
  ...
  const switchLanguage = () => {
    ...
    if (user?.role !== "guest") {
      void api
        .put("/api/settings", { language: next })
  ...
  return (
    <>
      <button
        ...
        aria-label={t("legacy.text_256a5c2ab314")}
        aria-expanded={open}
      ...
      <nav
        ...
        aria-label={t("legacy.text_c3815f2eb4af")}
      >
        <div className="nav-brand-row">
          <NavLink className="nav-brand" to="/dashboard/" onClick={() => setOpen(false)}>
            <div className="nav-logo-mark">
              <span className="nav-logo-iagents">{t("legacy.text_1fda9fc57a04")}</span>
              <span className="nav-logo-hub">{t("legacy.text_a38df5fc50fb")}</span>
            </div>
          </NavLink>

          <button
            className="nav-close-btn"
            aria-label={t("legacy.text_9f4dfc32f889")}
        ...
        {user?.role === "admin" && (
          <div className="nav-section nav-admin-section">
            ...
            <NavLink className="nav-link" to="/admin/metadata/">
              <span className="nav-link-icon">
                <NavIcon kind="logs" />
              </span>
              {t("admin.metadata.page_title")}
            </NavLink>

            <NavLink className="nav-link" to="/admin/centinel/">
              <span className="nav-link-icon">
                <NavIcon kind="centinel" />
              </span>
        ...
        <div className="nav-footer">
          <div className="nav-footer-actions">
            <button
              ...
              aria-label={t("legacy.text_8c5f1998de65")}
            >
              <NavIcon kind="lang" />
              <span className="nav-lang-label">{i18n.language.toUpperCase()}</span>
            </button>

            <button
              className={`nav-icon-btn${feedOpen ? " active" : ""}`}
              onClick={() => {
                setFeedOpen((value) => !value);
                setOpen(false);
              }}
            ...
            <NavLink className="nav-icon-btn" to="/labels/" title={t("labels.catalog.nav_title")}>
              <NavIcon kind="labels" />
            </NavLink>

            <NavLink className="nav-icon-btn" to="/docs/" title={t("nav.docs")}>
              <NavIcon kind="docs" />
            </NavLink>

            <NavLink className="nav-icon-btn" to="/about/" title={t("nav.about")}>
              <NavIcon kind="about" />
            </NavLink>

            <NavLink className="nav-icon-btn" to="/support/" title={t("nav.support")}>
              <NavIcon kind="support" />
            </NavLink>
          </div>
          ...
          <div className="nav-user-row">
            ...
            <button
              ...
              onClick={() => {
                if (confirm(t("nav.logout_confirm"))) logout.mutate();
              }}
