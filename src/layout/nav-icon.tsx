export function NavIcon({ kind, size = 15 }: { kind: string; size?: number }) {
  const common = {
    width: size,
    height: size,
    viewBox: "0 0 16 16",
    fill: "none",
    "aria-hidden": true,
  } as const;
  switch (kind) {
    case "dashboard":
      return (
        <svg {...common}>
          <rect
            x="1.5"
            y="1.5"
            width="5.5"
            height="5.5"
            rx="1.2"
            stroke="currentColor"
            strokeWidth="1.4"
          />
          <rect
            x="9"
            y="1.5"
            width="5.5"
            height="5.5"
            rx="1.2"
            stroke="currentColor"
            strokeWidth="1.4"
          />
          <rect
            x="1.5"
            y="9"
            width="5.5"
            height="5.5"
            rx="1.2"
            stroke="currentColor"
            strokeWidth="1.4"
          />
          <rect
            x="9"
            y="9"
            width="5.5"
            height="5.5"
            rx="1.2"
            stroke="currentColor"
            strokeWidth="1.4"
          />
        </svg>
      );
    case "agents":
      return (
        <svg {...common}>
          <rect x="2" y="7" width="12" height="8" rx="2" stroke="currentColor" strokeWidth="1.4" />
          <path d="M5 7V5a3 3 0 0 1 6 0v2" stroke="currentColor" strokeWidth="1.4" />
          <circle cx="8" cy="11" r="1.2" fill="currentColor" />
        </svg>
      );
    case "workflows":
      return (
        <svg {...common}>
          <circle cx="3" cy="3" r="1.5" stroke="currentColor" strokeWidth="1.3" />
          <circle cx="13" cy="8" r="1.5" stroke="currentColor" strokeWidth="1.3" />
          <circle cx="3" cy="13" r="1.5" stroke="currentColor" strokeWidth="1.3" />
          <path
            d="M4.5 3h2A2.5 2.5 0 0 1 9 5.5v0A2.5 2.5 0 0 0 11.5 8H9A2.5 2.5 0 0 0 6.5 10.5v0A2.5 2.5 0 0 1 4 13H4.5"
            stroke="currentColor"
            strokeWidth="1.2"
          />
        </svg>
      );
    case "knowledge":
      return (
        <svg {...common}>
          <path
            d="M3 2h7l3 3v9a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V3a1 1 0 0 1 1-1z"
            stroke="currentColor"
            strokeWidth="1.4"
            strokeLinejoin="round"
          />
          <path
            d="M10 2v3h3M5.5 8h5M5.5 10.5h4"
            stroke="currentColor"
            strokeWidth="1.2"
            strokeLinecap="round"
          />
        </svg>
      );
    case "connections":
      return (
        <svg {...common}>
          <circle cx="4" cy="4" r="2" stroke="currentColor" strokeWidth="1.4" />
          <circle cx="12" cy="4" r="2" stroke="currentColor" strokeWidth="1.4" />
          <circle cx="8" cy="13" r="2" stroke="currentColor" strokeWidth="1.4" />
          <path
            d="M4 6v2a4 4 0 0 0 4 4m0 0V6m0 6a4 4 0 0 0 4-4V6"
            stroke="currentColor"
            strokeWidth="1.3"
            strokeLinecap="round"
          />
        </svg>
      );
    case "explore":
      return (
        <svg {...common}>
          <circle cx="8" cy="8" r="6" stroke="currentColor" strokeWidth="1.4" />
          <path
            d="m10.5 5.5-2 4.5-4.5 2 2-4.5 4.5-2z"
            stroke="currentColor"
            strokeWidth="1.3"
            strokeLinejoin="round"
          />
          <circle cx="8" cy="8" r="1" fill="currentColor" />
        </svg>
      );
    case "feed":
      return (
        <svg {...common}>
          <path
            d="M2 4h12M2 8h8M2 12h5"
            stroke="currentColor"
            strokeWidth="1.5"
            strokeLinecap="round"
          />
        </svg>
      );
    case "labels":
      return (
        <svg {...common}>
          <path
            d="M2 5a1 1 0 0 1 1-1h6.5l4 4-4 4H3a1 1 0 0 1-1-1V5z"
            stroke="currentColor"
            strokeWidth="1.4"
            strokeLinejoin="round"
          />
          <circle cx="5.5" cy="8" r="1" fill="currentColor" />
        </svg>
      );
    case "docs":
      return (
        <svg {...common}>
          <path
            d="M2 3.5A2.5 2.5 0 0 1 4.5 2H8v12H4.5A2.5 2.5 0 0 0 2 15V3.5zM14 3.5A2.5 2.5 0 0 0 11.5 2H8v12h3.5A2.5 2.5 0 0 1 14 15V3.5z"
            stroke="currentColor"
            strokeWidth="1.35"
            strokeLinejoin="round"
          />
          <path
            d="M4.5 5h1.8M9.8 5h1.7"
            stroke="currentColor"
            strokeWidth="1.2"
            strokeLinecap="round"
          />
        </svg>
      );
    case "about":
      return (
        <svg {...common}>
          <circle cx="8" cy="8" r="6.25" stroke="currentColor" strokeWidth="1.4" />
          <path d="M8 7.2v4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
          <circle cx="8" cy="4.7" r=".85" fill="currentColor" />
        </svg>
      );
    case "support":
      return (
        <svg {...common}>
          <path
            d="M2.5 8a5.5 5.5 0 0 1 11 0"
            stroke="currentColor"
            strokeWidth="1.4"
            strokeLinecap="round"
          />
          <path
            d="M2.5 8.2v2.6a1.5 1.5 0 0 0 1.5 1.5h1V7.5H4A1.5 1.5 0 0 0 2.5 9M13.5 8.2v2.6a1.5 1.5 0 0 1-1.5 1.5h-1V7.5h1A1.5 1.5 0 0 1 13.5 9M11 12.3c-.4 1.1-1.3 1.7-3 1.7"
            stroke="currentColor"
            strokeWidth="1.35"
            strokeLinecap="round"
            strokeLinejoin="round"
          />
        </svg>
      );
    case "lang":
      return (
        <svg {...common}>
          <circle cx="8" cy="8" r="6" stroke="currentColor" strokeWidth="1.4" />
          <path
            d="M8 2c-1.5 2-2.5 3.8-2.5 6s1 4 2.5 6M8 2c1.5 2 2.5 3.8 2.5 6S9.5 14 8 14M2 8h12"
            stroke="currentColor"
            strokeWidth="1.2"
          />
        </svg>
      );
    case "admin":
      return (
        <svg {...common}>
          <circle cx="6" cy="5" r="2.5" stroke="currentColor" strokeWidth="1.4" />
          <path
            d="M1.5 13v-.5A4.5 4.5 0 0 1 6 8"
            stroke="currentColor"
            strokeWidth="1.4"
            strokeLinecap="round"
          />
          <circle cx="11.5" cy="10.5" r="3" stroke="currentColor" strokeWidth="1.4" />
          <path
            d="M11.5 9.2v1.3l.8.8"
            stroke="currentColor"
            strokeWidth="1.3"
            strokeLinecap="round"
          />
        </svg>
      );
    case "logs":
      return (
        <svg {...common}>
          <rect x="2" y="2" width="12" height="12" rx="2" stroke="currentColor" strokeWidth="1.4" />
          <path
            d="M4.5 5.5h7M4.5 8h5M4.5 10.5h6"
            stroke="currentColor"
            strokeWidth="1.2"
            strokeLinecap="round"
          />
        </svg>
      );
    case "centinel":
      return (
        <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden="true">
          <path
            d="M12 2L3 7v5c0 5.25 3.75 10.15 9 11.35C17.25 22.15 21 17.25 21 12V7L12 2z"
            stroke="currentColor"
            strokeWidth="1.4"
            strokeLinejoin="round"
          />
          <path
            d="M9 12l2 2 4-4"
            stroke="currentColor"
            strokeWidth="1.4"
            strokeLinecap="round"
            strokeLinejoin="round"
          />
        </svg>
      );
    default:
      return (
        <svg {...common}>
          <path
            d="M3 2h7l3 3v9a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V3a1 1 0 0 1 1-1z"
            stroke="currentColor"
            strokeWidth="1.4"
            strokeLinejoin="round"
          />
          <path d="M10 2v3h3" stroke="currentColor" strokeWidth="1.4" />
        </svg>
      );
  }
}
