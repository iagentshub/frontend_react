...
export function LoginPage() {
  ...
  const login = useMutation({
    mutationFn: () =>
      api.post<{ ok?: boolean }, { email: string; password: string }>("/api/auth/login", {
        email: email.trim(),
        password,
      }),
    onSuccess: async () => {
  ...
  const error =
    login.error instanceof ApiError
      ...
      : login.error
        ...
        : null;
  const showRegister =
    platform.data?.registration === "open" && platform.data.billing_enabled === false;
      ...
      <section className="login-card login-card--wide" aria-labelledby="login-title">
        ...
        <form autoComplete="on" noValidate onSubmit={submit}>
            ...
            <div className="field-pw">
              ...
              >
                <svg width="18" height="18" viewBox="0 0 18 18" fill="none" aria-hidden="true">
                  ...
                  {showPassword && <path d="M3 3l12 12" stroke="currentColor" strokeWidth="1.5" />}
                </svg>
              </button>
            </div>
          </div>
          ...
          <button type="submit" className="btn btn-primary btn-full" disabled={login.isPending}>
            {login.isPending ? t("auth.login_btn_loading") : t("auth.login_btn")}
          </button>

          <p style={{ textAlign: "right", margin: "8px 0 0" }}>
            <Link to="/forgot-password/" style={{ fontSize: 12, color: "var(--ink-2,#888)" }}>
        ...
        {(() => {
          const showGoogle = platform.data?.oauth_google_enabled !== false;
          const showApple = platform.data?.oauth_apple_enabled !== false;
          const showMicrosoft = platform.data?.oauth_microsoft_enabled !== false;
          ...
          return (
            <>
              ...
              <div className="login-oauth-row" aria-label={t("legacy.text_0377e2a41640")}>
                {showGoogle && (
                  <button
                    type="button"
                    className="login-oauth-btn"
                    disabled
                    title={t("about.contact.soon")}
                    aria-label={t("legacy.text_0f5998a4bd4a")}
                  >
                    <svg viewBox="0 0 18 18" aria-hidden="true">
                      ...
                      <path
                        fill="#FBBC05"
                        d="M3.964 10.71A5.1 5.1 0 0 1 3.682 9c0-.593.102-1.17.282-1.71V4.958H.957A9 9 0 0 0 0 9c0 1.452.348 2.827.957 4.042l3.007-2.332z"
                      />
                      <path
                        fill="#EA4335"
                ...
                {showApple && (
                  <button
                    ...
                    disabled
                    title={t("about.contact.soon")}
                    aria-label={t("legacy.text_59897c28a224")}
                  >
                ...
                {showMicrosoft && (
                  <button
                    ...
                    disabled
                    title={t("about.contact.soon")}
                    aria-label={t("legacy.text_2039b28bccbe")}
                  >
        ...
        {showRegister && (
          <p className="login-register-link">
            <span>{t("auth.register_text")}</span>
            <Link to="/register/">{t("auth.register_action")}</Link>
          </p>
        )}
