...
export function LoginPage() {
  ...
  // Botones de login social — placeholders deshabilitados ("Próximamente"),
  // visibles por defecto (oauth_*_enabled=true); el admin puede ocultarlos.
  const showGoogleOauth = platform.data?.oauth_google_enabled !== false;
  const showAppleOauth = platform.data?.oauth_apple_enabled !== false;
  const showMicrosoftOauth = platform.data?.oauth_microsoft_enabled !== false;
  const showOauthRow = showGoogleOauth || showAppleOauth || showMicrosoftOauth;
  ...
  return (
    <main className="login-wrap login-wrap--narrow">
      <section className="login-card login-card--wide" aria-labelledby="login-title">
        ...
        <form autoComplete="on" noValidate onSubmit={submit}>
          ...
          <div className="field">
            ...
            <div className="field-pw">
              ...
              <button type="button" className="pw-toggle" tabIndex={-1} onClick={() => setShowPassword((value) => !value)} aria-label={showPassword ? t("auth.hide_password") : t("auth.show_password")}>
        ...
        {showOauthRow && <div className="login-divider"><span>{t("auth.divider")}</span></div>}
        {showOauthRow && <div className="login-oauth-row" aria-label="Proveedores disponibles próximamente">
          {showGoogleOauth && <button type="button" className="login-oauth-btn" disabled title="Próximamente" aria-label="Google (próximamente)"><svg viewBox="0 0 18 18" aria-hidden="true"><path fill="#4285F4" d="M17.64 9.2c0-.637-.057-1.251-.164-1.84H9v3.481h4.844c-.209 1.125-.843 2.078-1.796 2.717v2.258h2.908c1.702-1.567 2.684-3.874 2.684-6.615z"/><path fill="#34A853" d="M9 18c2.43 0 4.467-.806 5.956-2.18l-2.908-2.259c-.806.54-1.837.86-3.048.86-2.344 0-4.328-1.584-5.036-3.711H.957v2.332C2.438 15.983 5.482 18 9 18z"/><path fill="#FBBC05" d="M3.964 10.71A5.1 5.1 0 0 1 3.682 9c0-.593.102-1.17.282-1.71V4.958H.957A9 9 0 0 0 0 9c0 1.452.348 2.827.957 4.042l3.007-2.332z"/><path fill="#EA4335" d="M9 3.58c1.321 0 2.508.454 3.44 1.345l2.582-2.58C13.463.891 11.426 0 9 0 5.482 0 2.438 2.017.957 4.958L3.964 7.29C4.672 5.163 6.656 3.58 9 3.58z"/></svg></button>}
          {showAppleOauth && <button type="button" className="login-oauth-btn" disabled title="Próximamente" aria-label="Apple (próximamente)"><svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/></svg></button>}
          {showMicrosoftOauth && <button type="button" className="login-oauth-btn" disabled title="Próximamente" aria-label="Microsoft (próximamente)"><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="1" y="1" width="10" height="10" fill="#F25022"/><rect x="13" y="1" width="10" height="10" fill="#7FBA00"/><rect x="1" y="13" width="10" height="10" fill="#00A4EF"/><rect x="13" y="13" width="10" height="10" fill="#FFB900"/></svg></button>}
        </div>}
