...
async function loadProfile(signal: AbortSignal): Promise<ProfileData> {
  ...
  const billing = platform.billing_enabled
    ? await safe<BillingState | null>("/api/billing/subscription", null)
    : null;
  return {
    session,
    social,
...
function NavIcon({ section }: { section: SectionId }) {
  if (section === "account" || section === "social")
    return (
  ...
  if (section === "workspaces")
    return (
      <svg width="15" height="15" viewBox="0 0 16 16" fill="none">
        ...
        <path d="M2 7h12M5 2v2M11 2v2" stroke="currentColor" strokeWidth="1.3" />
      </svg>
    );
  if (section === "tokens")
    return (
  ...
  if (section === "privacy")
    return (
      <svg width="15" height="15" viewBox="0 0 16 16" fill="none">
        <path
          d="M8 2L3 4.5V8c0 2.8 2.1 5.4 5 6 2.9-.6 5-3.2 5-6V4.5L8 2z"
          stroke="currentColor"
          strokeWidth="1.4"
        />
      </svg>
  ...
  return (
    <svg width="15" height="15" viewBox="0 0 16 16" fill="none">
      ...
      <path d="M8 2.5v11M2.5 8h11" stroke="currentColor" strokeWidth="1.3" />
    </svg>
  );
}
...
export function ProfilePage() {
  ...
  const nav = useMemo(
    ...
    [query.data?.platform.billing_enabled, t],
  );

  if (query.isPending)
    return (
      <main className="page-content">
        <div className="admin-empty">{t("legacy.text_a7ffd12534eb")}</div>
      </main>
    );
    ...
    return (
      <main className="page-content">
        <div className="admin-empty">
          <p>{t("legacy.text_91ac612df47f")}</p>
  ...
  return (
    <main className="page-content">
      <div className="page-header">
        <div>
          <h1 className="page-title">{t("common.nav.profile")}</h1>
          <p className="page-subtitle">{t("profile.page.subtitle")}</p>
        </div>
      </div>

      <div className="profile-panel">
        <nav className="profile-nav" aria-label={t("legacy.text_e719a71ebadc")}>
          <div className="profile-nav-user">
            <button
              className="profile-avatar"
              title={t("legacy.text_745309565227")}
              onClick={() => setParams({ tab: "account" })}
            >
              {avatarUrl ? (
                <img src={avatarUrl} alt={t("manager.manager.members.table_actions")} />
              ) : (
            ...
            <div className="profile-nav-user-info">
              ...
              <div className="profile-nav-badges">
                <span className="profile-role-badge">{roleLabel}</span>
                {session.auth_method && session.auth_method !== "internal" && (
                  <span className="profile-auth-badge">{session.auth_method}</span>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="profile-nav-items">
            {nav.map(([id, label]) => (
              <button
                key={id}
                className={`profile-nav-item${active === id ? " active" : ""}`}
        ...
        <div className="profile-sections">
          <section className="profile-section">
            {active === "account" && (
              <AccountSection session={session} onAvatarSaved={() => void query.refetch()} />
            )}
            ...
            {active === "privacy" && (
              <PrivacySection
                deletion={query.data.deletion}
                onReload={() => void query.refetch()}
              />
            )}

            {active === "billing" && (
              <BillingSection initial={query.data.billing} onReload={() => void query.refetch()} />
            )}
          </section>
        </div>
      </div>
